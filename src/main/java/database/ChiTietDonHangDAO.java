...
public class ChiTietDonHangDAO implements DAOInterface<ChiTietDonHang> {
	...
	public int deleteAll(DonHang dh) {
		...
		ArrayList<ChiTietDonHang> ChiTietDhList = this.selectAll();
		for(ChiTietDonHang ctdh : ChiTietDhList) {
			if(ctdh.getDonHang().getMaDonHang().equals(dh.getMaDonHang())) {
				ketQua += this.delete(ctdh);
			}
		}
